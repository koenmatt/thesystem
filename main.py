...
def get_related(parent_phrase):
    ...
    if r.status_code != 200:
        raise Exception(f"Scraper Aborted on phrase {parent_phrase}- over 10 requests")
...
def update_master(parent_phrase):
    master = pd.read_csv("master.csv")
    ind = list(master.index[master['Parent Phrase'] == parent_phrase])[0]
    master.at[ind, 'Harvested Related'] = True
    master.at[ind, 'Harvested alsoAsk'] = True
